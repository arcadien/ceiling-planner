...
spacing, plate dimensions, minimum offcut, joint mode). On a valid outline it returns HTTP 200
with the reconstructed vertices, the montant cut list, the rail cut list, the plate layout
(pieces plus summary), and a `totals` block giving the total montant length, the total rail
length, and the plate count. On an outline that fails validation it returns HTTP 400 with the
