...
montants twice), the total rail length, the total entretoise length, and the plate count, and a
`section` block giving the required span and the selected montant section for single and doubled
montants (null when the span exceeds the catalog). On an outline that fails validation it returns HTTP 400 with the
